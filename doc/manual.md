...
- ブラウザで開く
    ...
    http://my.hoge.net/projectplan/
    ```
- 参考にパッケージに含まれるsample.jsonを読み込んで開いてみる
...
- JSONファイルを読み込むこともできる
- CSVファイルからタスク情報を読み込み作成できる
  - [CSVファイルフォーマット](./csv_import.md)
- 保存済みファイルはリストから選択する
  - ステータスにより分類されリストされる
...
- 旧バージョン
  - バージョンを指定してLOADをクリックすると旧バージョンを開ける
  - ただし、参照のみで保存はできない

### タスク編集
![taskedit](./img/pm_taskedit.png)
- タスクの編集画面で、編集したい行をダブルクリックやENTERで編集モードになる。再度、ENTERで編集モード終わる
- 各項目の詳細は編集モーダルの説明を参照
...
- TOP/SUBのタスクは閉じる／開くことができる
  - 閉じた状態はガントチャートにも反映される
...
- 標準、狭いは列の幅を指定する
- グループのタスクを横に並べて表示する
![ganttchart](./img/pm_ganttchart2.png)
...
- JSONファイルのダウンロード、CSVファイルのダウンロードも可能。
